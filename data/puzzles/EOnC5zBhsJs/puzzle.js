// Title: LITS Killer
// Author: AFrayedKnot
// Video: https://www.youtube.com/watch?v=EOnC5zBhsJs
// Source: https://app.crackingthecryptic.com/sudoku/fLbtDgFRhm

// Rules encoded: normal sudoku over nine 9-cell jigsaw regions (not the
// default 3x3 boxes); seven X markers forcing their two cells to sum to 10;
// classic LITS shading -- one tetromino shaded per region, no 2x2 area
// entirely shaded, same free-tetromino shape (rotations/reflections counted
// the same) may not touch orthogonally across two different regions' shaded
// cells, and all 36 shaded cells across the grid form one connected area;
// plus each region's own shaded tetromino must sum to less than 13.
//
...
// rule 5 ("no 2x2 area may be entirely shaded") already forbids it globally,
// so excluding it from the per-region Or prunes an already-impossible branch
// rather than narrowing the rule.
//
// "Sum less than 13": any 4 distinct digits from 1-9 sum to at least
// 1+2+3+4=10 (the region's own all-different already forces distinctness
// within any 4-cell subset), so "less than 13" is exactly {10, 11, 12}.
...
const REGIONS = [
  ['R1C3', 'R1C2', 'R1C1', 'R2C1', 'R3C1', 'R3C2', 'R3C3', 'R4C3', 'R5C3'], // A
  ['R2C2', 'R2C3', 'R2C4', 'R3C4', 'R4C4', 'R3C5', 'R4C5', 'R4C6', 'R3C6'], // B
  ['R1C4', 'R1C5', 'R2C5', 'R2C6', 'R1C6', 'R1C7', 'R2C7', 'R3C7', 'R1C8'], // C
  ['R1C9', 'R2C9', 'R2C8', 'R3C8', 'R3C9', 'R4C9', 'R4C8', 'R4C7', 'R5C7'], // D
  ['R5C8', 'R5C9', 'R6C9', 'R6C8', 'R7C8', 'R6C7', 'R6C6', 'R6C5', 'R7C5'], // E
  ['R7C9', 'R8C9', 'R9C9', 'R8C8', 'R9C8', 'R7C7', 'R8C7', 'R9C7', 'R9C6'], // F
  ['R7C6', 'R8C6', 'R8C5', 'R9C5', 'R8C4', 'R9C4', 'R9C3', 'R9C2', 'R9C1'], // G
  ['R8C1', 'R8C2', 'R8C3', 'R7C3', 'R7C2', 'R7C1', 'R6C1', 'R5C1', 'R4C1'], // H
  ['R4C2', 'R5C2', 'R5C4', 'R5C5', 'R5C6', 'R6C2', 'R6C3', 'R6C4', 'R7C4'], // I
];
...
// One slack Var per region for the sum-under-13 rule: a single exact-equality
// Sum can stand in for "sum < 13" directly, instead of an Or of one equality
// Sum per feasible total. cells.sum + slack = 13 with slack restricted to
// {1, 2, 3} is exactly cells.sum in {12, 11, 10} -- "less than 13" (see
// header comment). The slack cell is fully determined by the four tetromino
// cells (slack = 13 - their sum), so it adds no free search state.
const slackVar = new Var('K', 'region tetromino sum-under-13 slack', REGIONS.length);
const slackDomain = slackVar.cells().map(cell => new Given(cell, 1, 2, 3));
...
function regionConstraint(regionIdx, regionCells) {
  ...
  const branches = tetrominoCandidates(regionCells).map(({ cells, shape }) => {
    ...
    const sumUnder13 = new Sum(13, ...cells, slackCell);
    return new And([...shadeGivens, new Given(shapeCell, SHAPE_ID[shape]), sumUnder13]);
  });
...
// --- X markers (sum to 10) -----------------------------------------------
// The puzzle's seven "X" edge markers, transcribed from the drawn geometry.
const xMarks = [
  ['R1C3', 'R1C4'],
  ['R6C3', 'R7C3'],
  ['R8C2', 'R8C3'],
