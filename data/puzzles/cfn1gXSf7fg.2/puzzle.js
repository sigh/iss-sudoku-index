...
// and arrow. Every clue cell carries a number and an arrow.
const clues = [
  { cell: 'R2C2', black: true, target: 4, dir: 'down' },
  { cell: 'R2C8', black: false, target: 2, dir: 'left' },
  { cell: 'R3C5', black: true, target: 2, dir: 'left' },
  { cell: 'R4C7', black: false, target: 1, dir: 'right' },
  { cell: 'R5C3', black: true, target: 0, dir: 'left' },
  { cell: 'R5C9', black: false, target: 1, dir: 'up' },
  { cell: 'R6C4', black: true, target: 1, dir: 'right' },
  { cell: 'R6C7', black: false, target: 1, dir: 'down' },
  { cell: 'R7C9', black: false, target: 5, dir: 'left' },
  { cell: 'R8C2', black: true, target: 3, dir: 'right' },
  { cell: 'R9C5', black: true, target: 2, dir: 'up' },
  { cell: 'R9C8', black: false, target: 1, dir: 'right' },
];
...
// A ray of exactly two cells is a binary relation, so it is a Pair with the
// same predicate (arms among the two cells === target).
const segmentCounts = clues.map(c => {
  ...
  if (ray.length === 2) {
    const armCount = (a, b) => (arms.includes(a) ? 1 : 0) + (arms.includes(b) ? 1 : 0);
    return new Pair(Pair.fnToKey((a, b) => armCount(a, b) === c.target, numValues),
      'segment count', ...ray);
  }
