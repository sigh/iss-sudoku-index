...
//
// Four of the numbers outside the grid sit in circles at the tail of an
// arrow running into the grid. They are read as sandwich totals rather than
// as shaft sums, because no sum reading survives the arithmetic: the circle
// carrying 21 has an arrow covering only R1C4 and R2C4, two cells of one
// column and so at most 9 + 8 = 17, and continuing that shaft to the grid
// edge instead makes it the whole of column 4, a constant 45.
//
// Not encoded:
//
// - The circled 23 above C3 and the circled 32 beside R4. The arrow attached
//   to each bends out of a single row or column part-way along -- down C3
//   then left along R5, and down the R5C1-R6C2-R7C3 diagonal then right
//   along R7 -- so the line each total labels could be the circle's own lane
//   or the bent path, and nothing drawn chooses between them.
// - The two empty circles outside the grid, above C6 and above C9: they
//   carry no number, so no total is given for their lines.
// - Whatever the six arrows entering the grid from the margin restrict
//   beyond naming the line their circle's total labels.
...
// along row 1, so both candidate lines for it are row 1; the rest are
// printed plainly beside the row or column they label.
