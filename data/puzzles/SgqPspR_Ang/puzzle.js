// Title: Anti Divisor Fillomino
// Author: SudokuExplorer
// Video: https://www.youtube.com/watch?v=SgqPspR_Ang
// Source: https://app.crackingthecryptic.com/sudoku/DG84dt9Q3M

// Anti Divisor Fillomino, 10x10. There is no Sudoku layer, so the grid is
// Raw: rows, columns and boxes carry no rule and values repeat freely.
//
// Rules encoded (the video description's wording, which replaces the
// payload's own "digit" with "number" for clarity -- sizes run past 9):
//  * Divide the grid into regions of orthogonally connected cells. Each cell
//    holds a number equal to the size of its own region.
//  * The size of any region must not divide the size of an orthogonally
//    adjacent region (neither direction -- this also forbids two adjacent
//    regions of equal size, since a size always divides itself).
//  * The 17 given numbers.
//
...
// its tens digit on an overlay and its units digit on the board. This is the
// split-number Fillomino construction validated on ZrfTSUxm0iE (Checkered
// Fillomino) and W6v-8Y6_Kkk/-qlhEDqO74k; only the inter-region edge rule
// differs here (anti-divisor instead of a checkered two-colouring).
...
// exactly when the two cells are in the same region. Within a region this
// forces one uniform number (the region's own size); across a boundary it
// forbids equal numbers -- already implied here by the anti-divisor rule
// below (a size always divides itself), but stated once so every cell in a
// region shows that region's own size rather than merely some number.
...
// Reads [tens(a), units(a), tens(b), units(b), rootRow(a), rootRow(b),
// rootCol(a), rootCol(b)] for one orthogonal edge. Once both full numbers are
// known (after the fourth read) the state collapses to a single "anti-divisor
// holds" flag, so the two two-digit magnitudes never have to be carried
// together with anything else -- keeping the compiled state count small.
// Accepts when the two regions differ (no rule between two cells of the same
// region) or neither number divides the other.
const antiDivisorEdgeSpec = NFA.encodeSpec({
  startState: { phase: 0 },
  transition: (state, value) => {
    if (state.phase === 0) return { phase: 1, tensA: value };
    if (state.phase === 1) return { phase: 2, valA: state.tensA * 10 + value };
    if (state.phase === 2) return { phase: 3, valA: state.valA, tensB: value };
    if (state.phase === 3) {
      const valB = state.tensB * 10 + value;
      const aDividesB = state.valA > 0 && valB % state.valA === 0;
      const bDividesA = valB > 0 && state.valA % valB === 0;
      return { phase: 4, ok: !aDividesB && !bDividesA };
    }
    if (state.phase === 4) return { phase: 5, ok: state.ok, mine: value };
    if (state.phase === 5) {
      return { phase: 6, ok: state.ok, rowSame: value === state.mine };
    }
    if (state.phase === 6) {
      return { phase: 7, ok: state.ok, rowSame: state.rowSame, mine: value };
    }
    if (state.phase === 7) {
      const sameRegion = state.rowSame && value === state.mine;
      return { phase: 8, pass: state.ok || sameRegion };
    }
    return undefined;
  },
  accept: state => state.phase === 8 && state.pass,
}, shape);
...
const edgeRules = edges.flatMap(([a, b]) => [
  ...
  new NFA(antiDivisorEdgeSpec, 'anti-divisor between adjacent regions',
    tens.at(a), a, tens.at(b), b,
    rootRow.at(a), rootRow.at(b), rootCol.at(a), rootCol.at(b)),
]);

// Transcribed from the 17 numbers printed in the grid: [row, col, number].
const GIVENS = [
  [1, 2, 2], [1, 6, 5], [2, 4, 4], [3, 3, 6], [3, 7, 9], [3, 9, 2],
  [4, 2, 9], [4, 5, 2], [5, 4, 3], [5, 8, 3], [6, 7, 5], [6, 9, 3],
  [7, 3, 3], [8, 9, 5], [9, 2, 6], [9, 6, 2], [10, 9, 6],
];
