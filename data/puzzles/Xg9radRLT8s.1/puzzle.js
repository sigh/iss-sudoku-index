...
// Source: http://pzv.jp/p.html?akari/9/9/h.bj.i1.cah.j..hbp1..h.bgbaibam..g
...
// "shading as a widened Raw grid" technique used for this pipeline's prior
// Akari rows (huMgnsAIDjw.1 etc). There is no separate "is illuminated"
// overlay: a white cell's row-run and column-run (the maximal stretch of
// white cells either side of it, up to the nearest wall or edge) are
// computed directly from the fixed wall layout, and illumination becomes "at
// least one cell in that union is a bulb" -- a single Or of Given(cell, BULB)
// branches, one branch per candidate cell, which is trivially true when the
// cell is itself a bulb.
...
// The 22 black cells, transcribed from the source's own clue table
...
// cell of the 9x9 grid is a white (playable) cell.
const WALLS = [
  [1, 3, -2], [1, 4, 1],
  [2, 2, -2], [2, 6, 1], [2, 7, -2], [2, 8, 2],
  [3, 2, 0], [3, 7, -2],
  [4, 3, -2], [4, 4, -2], [4, 7, 1],
  [6, 2, 1], [6, 3, -2], [6, 4, -2], [6, 7, -2], [6, 8, 1],
  [7, 3, 1], [7, 6, 0],
  [8, 3, 1], [8, 6, 0],
  [9, 7, -2], [9, 8, -2],
];
