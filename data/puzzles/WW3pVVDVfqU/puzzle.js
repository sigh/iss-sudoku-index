...
//
// The main grid is Raw: no implicit row/column/box constraints, so its 0
// (blank) cells don't conflict with the manual all-different below. VL and VO
// are auxiliary Var overlays over the same 11x11 cell graph:
...
const circleCounts = CIRCLES.flatMap(cell => {
  ...
  for (let l = 1; l <= 9; l++) {
    ...
    branches.push(new Or([
      ...
      new NFA(countMachine, `circle${cell}L${l}`, cell, ...loop.at(block)),
    ]));
  }
  return branches;
});
...
const palindromes = [
  new Palindrome(...LINE_A),
  new Palindrome(...LINE_B),
];
