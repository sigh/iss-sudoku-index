// Title: Castle Wall
// Author: Ken Endo
// Video: https://www.youtube.com/watch?v=lA1_IXku-ts
...
// are not on the loop and lie outside it; white bordered cells would lie
// inside it and grey cells could be either, but this board has neither. A
// number with an arrow counts the cell borders in the clue's own row or
// column, on the arrowed side of the clue, that the loop passes through --
// i.e. the loop segments lying along that lane.
...
// Clue table, transcribed from the drawn cells: the whole of row 1 and
// column 1 is black; every row-1 number points down, every column-1 number
// points right. R1C1 is black with no number. Cell ids are built with
// makeCellId because column 10 is not spelled "C10".
const clues = [
  { row: 1, col: 1 },
  { row: 1, col: 2, target: 1, dir: 'down' },
  { row: 1, col: 3, target: 2, dir: 'down' },
  { row: 1, col: 4, target: 4, dir: 'down' },
  { row: 1, col: 5, target: 6, dir: 'down' },
  { row: 1, col: 6, target: 1, dir: 'down' },
  { row: 1, col: 7, target: 2, dir: 'down' },
  { row: 1, col: 8, target: 5, dir: 'down' },
  { row: 1, col: 9, target: 2, dir: 'down' },
  { row: 1, col: 10, target: 3, dir: 'down' },
  { row: 2, col: 1, target: 1, dir: 'right' },
  { row: 3, col: 1, target: 4, dir: 'right' },
  { row: 4, col: 1, target: 3, dir: 'right' },
  { row: 5, col: 1, target: 6, dir: 'right' },
  { row: 6, col: 1, target: 1, dir: 'right' },
  { row: 7, col: 1, target: 2, dir: 'right' },
  { row: 8, col: 1, target: 5, dir: 'right' },
  { row: 9, col: 1, target: 2, dir: 'right' },
  { row: 10, col: 1, target: 2, dir: 'right' },
].map(c => ({ ...c, cell: makeCellId(c.row, c.col) }));

// Every clue cell is black: off the loop and outside it.
const clueCells = clues.map(c => new Given(c.cell, OUTSIDE));
