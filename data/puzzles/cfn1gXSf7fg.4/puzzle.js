...
// the loop and lie inside it (this puzzle has none). A number with an arrow
// counts the loop segments in the clue's own row or column on the arrowed
// side of the clue, i.e. the cell borders in that ray the loop passes
// through. Every rule is encoded; nothing is omitted.
//
...
// Clue table, transcribed from the drawn cells: every cell of row 1 and of
// column 1 is black; each except the corner R1C1 carries a number whose
// arrow points into the grid (down from row 1, right from column 1). Cell
// ids are built with makeCellId because column 10 is not spelled "C10".
const clueTable = [
  { row: 1, col: 1 },
...
const clues = clueTable.map(c => ({ ...c, cell: makeCellId(c.row, c.col), black: true }));
