// Title: Fillomino XV
// Author: Au Voleur!
// Video: https://www.youtube.com/watch?v=wYttOMMuiJc
// Source: https://app.crackingthecryptic.com/sudoku/P44Rj4NGhP

// Fillomino XV, 10x10. There is no Sudoku layer, so the grid is Raw: rows,
...
//    region may contain zero, one or more cells.
//  * V/X: the numbers in two cells joined by a printed V sum to 5; joined by
//    a printed X sum to 10. "All possible Xs and Vs are given" -- every
//    orthogonally adjacent pair NOT carrying a V or X must sum to neither 5
//    nor 10.
//  * No printed numbers at all: every cell's area value is unclued.
//
...
// BFS-distance residues), the construction from data/puzzles/ZrfTSUxm0iE and
// data/puzzles/guTXBSiStkw (blocker #1618's 2026-09-05 amendment). The V/X
// rule reuses the same tens+units pair already read for the Fillomino edge
// rule: a sum of 5 or 10 only ever involves two cells whose numbers are both
// under 10 (the smallest addend is 1), so no new layer is needed for it.
...
// V/X: reads [tens(a), tens(b), units(a), units(b)] of one orthogonal edge and
// checks the two cells' full numbers (10*tens+units each) against a required
// condition on their sum. A number over 10 can never take part in a sum of 5
// or 10 (the other addend is never negative), so once a number's tens and
// units combine past 10 it is folded to one sentinel value (`CAP`) rather
// than kept exact. That keeps the machine's state count small: without the
// fold it would have to track each number's full raw range exactly.
const CAP = 11;
const capNum = n => Math.min(n, CAP);
const sumEdgeSpec = require => NFA.encodeSpec({
  startState: { phase: 0 },
  transition: (state, value) => {
    if (state.phase === 0) return { phase: 1, tensA: value };
    if (state.phase === 1) return { phase: 2, tensA: state.tensA, tensB: value };
    if (state.phase === 2) {
      const numA = capNum(10 * state.tensA + value);
      return { phase: 3, numA, tensB: state.tensB };
    }
    if (state.phase === 3) {
      const numB = capNum(10 * state.tensB + value);
      return require(state.numA + numB) ? { phase: 4 } : undefined;
    }
    return undefined;
  },
  accept: state => state.phase === 4,
}, shape);
const vSumSpec = sumEdgeSpec(sum => sum === 5);
const xSumSpec = sumEdgeSpec(sum => sum === 10);
// "All possible Xs and Vs are given": every unmarked edge sums to neither.
const noSumSpec = sumEdgeSpec(sum => sum !== 5 && sum !== 10);

// Transcribed from the 18 V/X letters drawn on the grid edges (7 V, 11 X):
// each pair names the two R#C# cells the letter sits between.
const V_EDGES = [
  [[1, 1], [1, 2]], [[1, 2], [1, 3]], [[10, 2], [10, 3]],
  [[3, 10], [4, 10]], [[4, 10], [5, 10]], [[4, 5], [4, 6]], [[3, 4], [4, 4]],
];
const X_EDGES = [
  [[3, 1], [3, 2]], [[8, 1], [9, 1]], [[8, 10], [9, 10]], [[8, 9], [8, 10]],
  [[8, 8], [8, 9]], [[8, 7], [9, 7]], [[6, 7], [6, 8]], [[6, 5], [6, 6]],
  [[2, 5], [3, 5]], [[2, 7], [2, 8]], [[1, 9], [2, 9]],
];
const markKey = ([r1, c1], [r2, c2]) => `${r1}_${c1}_${r2}_${c2}`;
const marks = new Map();
for (const [p, q] of V_EDGES) marks.set(markKey(p, q), 'V');
for (const [p, q] of X_EDGES) marks.set(markKey(p, q), 'X');

const sumRules = edges.map(([a, b]) => {
  const { row: r1, col: c1 } = parseCellId(a);
  const { row: r2, col: c2 } = parseCellId(b);
  const mark = marks.get(markKey([r1, c1], [r2, c2]));
  const spec = mark === 'V' ? vSumSpec : mark === 'X' ? xSumSpec : noSumSpec;
  const name = mark === 'V' ? 'V: numbers sum to 5'
    : mark === 'X' ? 'X: numbers sum to 10'
      : 'unmarked edge: numbers sum to neither 5 nor 10';
  return new NFA(spec, name, tens.at(a), tens.at(b), a, b);
});
