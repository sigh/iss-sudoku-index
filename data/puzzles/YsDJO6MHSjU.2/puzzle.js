...
// Source: https://cracking-the-cryptic.web.app/sudoku/BH49r9mBng

// Pentominous on the white 10x10 grid. Rules encoded here, in full:
//  * Divide the grid into pentominoes (five-cell orthogonally connected
//    regions).
//  * A cell carrying a letter lies in a pentomino of that letter's shape, as
//    drawn in the inventory below the grid, up to rotation and reflection.
//  * Two pentominoes of the same shape may not share an edge.
// "Not all shapes must be used" states that the inventory is a catalogue with
// unlimited reuse, so it adds no constraint and none is written for it.
//
...
// grid is `Raw`: a letter repeats freely along a row or column, and there are
// no boxes.
//
// Because same-shape pentominoes may not touch, the pentominoes ARE the
...
// takes, VK is which cell of that orientation this cell is. (letter, VO, VK)
// give the offset, and the two overlays are functions of the true piece, so
// they add no symmetry of their own and need no pinning.

// The twelve coloured blocks of the inventory below the grid, as drawn, each
...
// The inventory shows twelve shapes -- the whole free-pentomino set, with each
// of the six chiral pentominoes appearing once and no separate letter for its
// mirror -- and the rules say "including rotations/reflections". A letter
// therefore names its shape up to rotation and reflection, so all eight
// symmetries are generated here and deduplicated. ORIENTS[letter index] lists
// that letter's distinct fixed forms.
...
// --- Same-shape pieces may not share an edge ----------------------------
...
// step between them, which says the two cells belong to the same piece. So two
// distinct pieces of the same letter can never be edge-adjacent.
...
// --- Lettered cells -----------------------------------------------------
// The eight letters printed in the white grid, from the drawn cell values, as
// [row, column, letter]. Row and column 10 are outside the R#C# digit range,
// so cell ids are built with makeCellId rather than written out.
const CLUES = [
  [2, 4, 'F'], [2, 7, 'F'],
  [5, 4, 'W'], [5, 7, 'Z'],
  [8, 1, 'W'], [8, 10, 'Z'],
...
const clues = CLUES.map(([row, col, letter]) =>
  new Given(makeCellId(row, col), LETTERS.indexOf(letter) + 1));
