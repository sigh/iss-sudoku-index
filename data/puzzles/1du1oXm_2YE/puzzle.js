// Title: Hybrid Star Battle
// Author: BenceJoful
// Video: https://www.youtube.com/watch?v=1du1oXm_2YE
// Source: https://app.crackingthecryptic.com/sudoku/QtRQHrf8R9

// Hybrid Star Battle, 10x10. There is no Sudoku layer, so the grid is Raw:
// rows, columns and boxes carry no rule and values repeat freely.
//
// Rules encoded:
//  * Fillomino. Divide the grid into orthogonally-connected regions. Two
//    regions of the same size do not share an edge. A clued cell belongs to
//    a region of the printed size; unclued cells carry no size of their own.
//  * The 17 given "5"s.
//  * Star battle over the same regions: shade some cells so every row,
//    column and region holds exactly two shaded cells, and no two shaded
//    cells touch, including diagonally.
//
// Nothing is omitted. A region's size runs to the board's own cap of 100
// cells, so it does not fit in one 16-value cell: every cell's number is
// held as its tens digit on an overlay and its units digit on the board
// (construction per data/puzzles/ZrfTSUxm0iE/puzzle.js).
...
// root is its first cell in reading order. Six overlays carry the model:
//   tens     - tens digit of the cell's region-size number (board = units);
//   rootRow,
...
//   star     - shaded (1) or unshaded (0), the star-battle layer.
...
// region one step nearer the root. Following such neighbours changes the
// residue pair by one each step, so the walk cannot revisit a cell within
// 143 steps and must reach a root: the region is connected and contains the
// cell it names.
...
// nobody -- this half of the invariant is what a non-root branch checks.
...
// exactly when the two cells are in the same region. Across a boundary this
// is exactly "regions of equal size do not touch orthogonally".
...
// --- Star battle over the discovered regions ---

// Rows and columns are literal cell lists, so their star count is a plain
// ContainExact. "1_1" over a {0,1} alphabet: exactly two 1s, everything else
// forced to 0.
const TWO_STARRED = '1_1';
const rowColStars = [...graph.rows(), ...graph.columns()].map(
  house => new ContainExact(TWO_STARRED, ...star.at(house)));

// No two king-move-adjacent cells are both starred. Each template spans only
// the VS overlay, so Replicate must locate anchor/targets in that overlay's
// own cell group, not the main grid's.
const notBothStarred = Pair.fnToKey((a, b) => a === 0 || b === 0, shape);
const TOUCHING_OFFSETS = [[0, 1], [1, 0], [1, 1], [1, -1]];
const noTouchPairs = TOUCHING_OFFSETS.map(([dRow, dCol]) => {
  const origins = graph.cells().filter(cell => graph.step(cell, dRow, dCol));
  const anchor = star.at(origins[0]);
  const partner = star.at(graph.step(origins[0], dRow, dCol));
  return new Replicate(
    [new Pair(notBothStarred, 'stars do not touch', anchor, partner)],
    Replicate.encodeTargetCells(star.at(origins), anchor, star),
    anchor);
});

// A region's star count is exactly two. Reads [d11(cell), d13(cell), then
// rootRow, rootCol and star of this cell and of every cell after it in
// reading order]. A cell at distance 0 is a root: exactly two of the cells
// naming it (itself included, since a root always names itself) are
// starred. A cell at positive distance is not a root; "nobody names it"
// is already enforced by the region-size NFA above, so its branch just
// consumes the rest of the input unconditionally.
const starSpecs = new Map();
const starSpec = (row, col) => {
  const key = row + '_' + col;
  if (!starSpecs.has(key)) {
    starSpecs.set(key, NFA.encodeSpec({
      startState: { phase: 'd11' },
      transition: (state, value) => {
        if (state.phase === 'd11') return { phase: 'd13', zero: value === 0 };
        if (state.phase === 'd13') {
          const isRoot = state.zero && value === 0;
          return isRoot ? { phase: 'row', rem: 0 } : { phase: 'pass' };
        }
        ...
        if (state.phase === 'col') {
          return { phase: 'star', rem: state.rem, match: state.rowEq && value === col };
        }
        // phase === 'star'
        const rem = state.match && value === 1 ? state.rem + 1 : state.rem;
        return rem > 2 ? undefined : { phase: 'row', rem };
      },
      accept: state => state.phase === 'pass' || (state.phase === 'row' && state.rem === 2),
    }, shape));
  }
  return starSpecs.get(key);
};

const regionStars = cells.map((cell, i) => {
  const { row, col } = parseCellId(cell);
  const later = cells.slice(i);
  return new NFA(starSpec(row, col), 'region contains exactly two shaded cells',
    d11.at(cell), d13.at(cell),
    ...later.flatMap(other => [rootRow.at(other), rootCol.at(other), star.at(other)]));
});

// Transcribed from the 17 "5"s printed in the grid: [row, col].
const GIVEN_CELLS = [
  [1, 4], [1, 9], [2, 3], [2, 8], [3, 2], [3, 4], [3, 9], [4, 3], [4, 8],
  [5, 2], [5, 6], [5, 9], [6, 1], [6, 7], [7, 10], [8, 5], [10, 9],
];
const givens = GIVEN_CELLS.flatMap(([row, col]) => {
