// Title: Fillomino
// Author: Grant Fikes
// Video: https://www.youtube.com/watch?v=I2qTNJoLD1o
// Source: http://pzv.jp/p.html?fillomino/10/10/j27j5j2h7k51h553i1h551h3t2h473h6i253h48k1h4j8j18j

// Fillomino, 10x10. There is no Sudoku layer, so the grid is Raw: rows,
...
//  * The 30 given numbers.
//
// Nothing is omitted. A region's size is not bounded by any rule or drawn
// clue, so a cell's number is held as its tens digit on an overlay and its
// units digit on the board, letting a region run to the board's own cap of
// 100 cells.

const SIDE = 10;
const MAX_AREA = SIDE * SIDE;               // 100
...
//                      and mod 13 (lcm 143 > 100, so the pair is the distance
//                      itself).
...
// exactly when the two cells are in the same region. Across a connected
// region this makes the number uniform; across a boundary it forbids two
// touching regions from sharing a size.
...
// Transcribed from the 30 numbers printed in the grid: [row, col, number].
const GIVENS = [
  [1, 5, 2], [1, 6, 7],
  [2, 1, 5], [2, 6, 2], [2, 9, 7],
  [3, 5, 5], [3, 6, 1], [3, 9, 5], [3, 10, 5],
  [4, 1, 3], [4, 5, 1], [4, 8, 5], [4, 9, 5], [4, 10, 1],
  [5, 3, 3],
  [6, 8, 2],
  [7, 1, 4], [7, 2, 7], [7, 3, 3], [7, 6, 6], [7, 10, 2],
  [8, 1, 5], [8, 2, 3], [8, 5, 4], [8, 6, 8],
  [9, 2, 1], [9, 5, 4], [9, 10, 8],
  [10, 5, 1], [10, 6, 8],
];
