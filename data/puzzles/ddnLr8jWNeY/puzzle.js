// Title: Hypnotic Suggestion
// Author: Atticus837
// Video: https://www.youtube.com/watch?v=ddnLr8jWNeY
// Source: https://sudokupad.app/qt0xjevecl

// Each clue is read as (count, digit) pairs. Values not named by the clue are
// unrestricted.
const cageSpecs = [
  {
    cells: [
      'R1C1', 'R1C2', 'R1C3', 'R1C4', 'R1C5', 'R1C6', 'R1C7', 'R1C8',
      'R2C8',
      'R3C2', 'R3C3', 'R3C4', 'R3C5', 'R3C6', 'R3C8',
      'R4C2', 'R4C6', 'R4C8',
      'R5C2', 'R5C4', 'R5C6', 'R5C8',
      'R6C2', 'R6C4', 'R6C5', 'R6C6', 'R6C8',
      'R7C2', 'R7C8',
      'R8C2', 'R8C3', 'R8C4', 'R8C5', 'R8C6', 'R8C7', 'R8C8',
    ],
    counts: [[8, 7], [6, 5], [4, 3], [2, 1]], // 87654321
  },
  {
    cells: [
      'R2C1', 'R2C2', 'R2C3', 'R2C4', 'R2C5', 'R2C6', 'R2C7',
      'R3C1', 'R3C7',
      'R4C1', 'R4C3', 'R4C4', 'R4C5', 'R4C7',
      'R5C1', 'R5C3', 'R5C5', 'R5C7',
      'R6C1', 'R6C3', 'R6C7',
      'R7C1', 'R7C3', 'R7C4', 'R7C5', 'R7C6', 'R7C7',
      'R8C1',
    ],
    counts: [[1, 2], [3, 4], [5, 6], [7, 8]], // 12345678
  },
];

const lookAndSayClues = cageSpecs.map(({ cells, counts }) =>
  new LookAndSay(counts.flat().join(''), ...cells));

return [
  new Shape('8x8'),
  ...lookAndSayClues,
];
