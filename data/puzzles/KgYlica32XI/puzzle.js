...
// Nothing is omitted.
//
// Rows/columns can repeat 0s, so the grid is Raw: no implicit constraints.
...
// One overlay per per-cell unknown (except the digit, now the main grid), all
// indexed by the grid cell they shadow.
const T = graph.makeOverlay('VT');  // 1 = a box has its top-left corner here, 2 = no
const F = graph.makeOverlay('VF');  // 1 = this cell lies inside a box, 2 = no
const P = graph.makeOverlay('VP');  // this cell's step (1-9) along its box's snake,
...
const corners = cells.filter(cell => graph.block(cell, 3, 3) !== null);
const cornerRoom = T.makeReplicate(
  new Given(T.cells()[0], 2),
  T.at(cells.filter(cell => graph.block(cell, 3, 3) === null)));

// A cell holds a digit/step exactly when it lies inside a box.
const zeroKey = Pair.fnToKey((value, f) => (value !== BLANK) === (f === 1), shape);
const filledD = cells.map(cell => new Pair(zeroKey, 'filled-digit', cell, F.at(cell)));
...
const coverage = cells.map(cell => {
  ...
  for (let dRow = -2; dRow <= 0; dRow++) for (let dCol = -2; dCol <= 0; dCol++) {
    const corner = graph.step(cell, dRow, dCol);
...
const directionKey = Pair.fnToKey((a, b) => a < b, shape);

const boxRules = corners.map(corner => {
  const block = graph.block(corner, 3, 3);
  const blockSet = new Set(block);
  const snakeSteps = block.map(cell => {
    const neighbourCells = graph.neighbours(cell).filter(n => blockSet.has(n));
    return new NFA(predecessorAndSum, 'snake', cell, P.at(cell),
      ...neighbourCells.flatMap(n => [P.at(n), n]));
  });
  return new Or([
...
// it directly. ---
const rowColKey = PairX.fnToKey((a, b) => a !== b || a === BLANK, shape);
...
const cageRules = CAGES.flatMap(({ total, cells: cageCells }) => [
  new PairX(rowColKey, 'no-repeat', ...cageCells),
  new Sum(total, ...cageCells),
]);

// The one given, read off the payload's `cells` array.
const givenR3C5 = new Given('R3C5', 1);

return [
  shape,
  T.toVar('T'), F.toVar('F'), P.toVar('P'),
  ...domains, cornerRoom,
