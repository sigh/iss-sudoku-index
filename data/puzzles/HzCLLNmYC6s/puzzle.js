// Auralis by Rab3aron
// https://sudokupad.app/8s7d1v9agq
// https://www.youtube.com/watch?v=HzCLLNmYC6s
//
// Normal sudoku. Blue lines are region sum lines. Yellow dots join
// nonconsecutive digits.

const lines = [
  ['R4C3', 'R5C4', 'R6C4', 'R7C5', 'R6C5', 'R6C6'],
  ['R6C8', 'R6C7', 'R5C6', 'R5C7', 'R4C7', 'R4C6', 'R5C5'],
  ['R4C9', 'R5C9', 'R6C9', 'R7C9', 'R7C8'],
  ['R2C5', 'R3C5', 'R4C5', 'R4C4', 'R3C3', 'R3C2', 'R4C2', 'R4C1', 'R3C1', 'R2C1'],
  ['R7C1', 'R6C1', 'R6C2', 'R7C2', 'R7C3', 'R7C4', 'R8C4'],
  ['R3C6', 'R3C7', 'R2C7'],
  ['R8C6', 'R9C7'],
];

const yellowDots = [
  ['R7C8', 'R7C9'],
  ['R2C5', 'R3C5'],
  ['R4C1', 'R4C2'],
  ['R4C8', 'R5C8'],
  ['R6C8', 'R7C8'],
  ['R6C2', 'R6C3'],
  ['R2C1', 'R2C2'],
  ['R2C6', 'R3C6'],
];

const nonconsecutiveKey = Pair.fnToKey((a, b) => Math.abs(a - b) > 1, 9);

return [
  new Shape('9x9'),
  ...lines.map(cells => new RegionSumLine(...cells)),
  ...yellowDots.map(cells => new Pair(nonconsecutiveKey, 'non-consecutive', ...cells)),
];
