...
// Nothing is omitted.
//
// The answer cannot live on a default Sudoku-type grid. Main-grid rows and
// columns are unconditionally all-different, and the only seven-cell sets
// these rules force apart are the seven regions and the fifteen full-length
// lines; those 22 sets admit exactly one partition of the 49 cells (the
// regions themselves), so no placement on a 7x7 grid makes both the grid's
// rows and its columns rule-forced groups. The board is instead a Raw 9x7
// canvas (one canvas row per board row), which carries no implicit
// constraints at all. The board's rows are ragged, so the canvas has 14 cells
// that are not hexagons; the value range is widened to 8 to mark them.
...
const shape = new Shape('9x7', OFF_BOARD, 'Raw');
const canvas = cellGraph(shape);

const xOf = (row, position) => ROWS[row - 1][0] + 2 * (position - 1);
// The main-grid cell at board coordinates (row, x), or null when x is off the
// board or has the wrong parity for that row.
const cellAt = (row, x) => {
  ...
  return makeCellId(row, offset / 2 + 1);
};

const hexagons = ROWS.flatMap(([first, width], i) =>
  Array.from({ length: width }, (_, j) => cellAt(i + 1, first + 2 * j)));
const spare = canvas.cells().filter(cell => !hexagons.includes(cell));
...
return [
  ...
  canvas.makeReplicate(new Given(hexagons[0], 1, 2, 3, 4, 5, 6, 7), hexagons),
