// Title: chi
// Author: MOORESUDOKU90
// Video: https://www.youtube.com/watch?v=ovnLIQzpdDY
// Source: https://sudokupad.app/6jqxfbbrtz

// Normal killer sudoku: cage digits do not repeat, and each cage's sum is
// the two-digit number formed by its first two cells in grid reading order
// (the cages are self-clued; no totals are printed). Digits on the green
// line are even, digits on the blue line are odd, and digits do not repeat
// on a line. Cells joined by an X sum to 10.

const cages = [
  ["R3C8", "R3C9", "R4C9", "R5C9", "R6C9"],
  ["R7C8", "R7C9", "R8C9", "R9C9"],
  ["R7C3", "R7C4", "R8C3", "R8C4", "R8C5", "R9C3"],
  ["R4C2", "R4C3", "R5C3", "R6C3"],
  ["R2C1", "R3C1", "R4C1", "R5C1", "R6C1"],
  ["R1C3", "R2C3", "R3C3", "R3C4"],
  ["R2C4", "R2C5", "R3C5"],
  ["R1C4", "R1C5", "R1C6", "R2C6", "R3C6"],
  ["R4C5", "R5C4", "R5C5", "R5C6", "R6C4"],
  ["R6C6", "R7C5", "R7C6"],
  ["R5C7", "R5C8", "R6C7", "R7C7"],
  ["R9C5", "R9C6", "R9C7", "R9C8"],
];

const greenEvenLine = ["R4C1", "R3C2", "R2C3", "R1C4"];
const blueOddLine = ["R9C5", "R8C6", "R7C7", "R6C8", "R5C9"];

function selfCluedCage(cells) {
  const ordered = [...cells].sort((a, b) => {
    const cellA = parseCellId(a);
    const cellB = parseCellId(b);
    return cellA.row - cellB.row || cellA.col - cellB.col;
  });
  const [tensCell, onesCell] = ordered;
  return new And([
    new AllDifferent(...cells),
    new PillArrow(2, tensCell, onesCell, ...cells),
  ]);
}

return [
  new Shape("9x9"),
  ...cages.map(selfCluedCage),
  new AllDifferent(...greenEvenLine),
  ...greenEvenLine.map(cell => new Given(cell, 2, 4, 6, 8)),
  new AllDifferent(...blueOddLine),
  ...blueOddLine.map(cell => new Given(cell, 1, 3, 5, 7, 9)),
  new X("R2C6", "R2C7"),
];
