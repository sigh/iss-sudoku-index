// Title: An Easter Miracle
// Author: Farkov and TopAutism
// Video: https://www.youtube.com/watch?v=prNvphj3UGg
// Source: https://app.crackingthecryptic.com/sudoku/3BqbqHQq3B
...
// - Orthogonally adjacent digits may not be consecutive (global rule, not
//   scoped to the marked cells).
// - In boxes 1, 2 and 3: for every digit d, at least one of the three boxes
//   holds d at its own reading-order position d.
...
// Boxes 1, 2 and 3: for each digit d, at least one of the three boxes holds
// digit d at its own reading-order position d (a fixed cell, once the box is
// fixed).
const fixedPointRule = [];
for (let d = 1; d <= 9; d++) {
  fixedPointRule.push(new Or([
    new Given(boxes[0][d - 1], d),
    new Given(boxes[1][d - 1], d),
    new Given(boxes[2][d - 1], d),
  ]));
}
